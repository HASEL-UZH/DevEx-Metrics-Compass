...
# Machine-readable map of the generated pages, loaded by the app's small-screen gate
# so it can point phone visitors at the landing page matching their deep link.
LIBRARY_MAP_NAME = "library-map.js"
...
# ─── Library map (consumed by the app's small-screen gate) ───────────────────

def write_library_map(framework_slugs, company_slugs, metric_slugs, metrics):
    """Emit library-map.js: deep-link params -> landing page slug.

    The compass is gated on phones (the sunburst needs a wide screen). Rather
    than dead-ending a phone visitor who followed a deep link, the gate looks
    the link's filter up here and offers the matching static landing page.
    """
    payload = {
        "frameworks": framework_slugs,
        "companies": company_slugs,
        "metrics": {str(m["id"]): {"slug": metric_slugs[m["id"]], "name": m["name"]}
                    for m in metrics if m["id"] in metric_slugs},
    }
    lines = ["// Generated by dataset/generate_seo_pages.py - do not edit by hand.",
             "window.LIBRARY_MAP = "
             + json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True) + ";",
             ""]
    js = chr(10).join(lines)
    with open(os.path.join(LIBRARY_DIR, LIBRARY_MAP_NAME), "w", encoding="utf-8") as fh:
        fh.write(js)
...
def main():
    ...
    write_library_map(framework_slugs, company_slugs, page_slugs, metrics)
